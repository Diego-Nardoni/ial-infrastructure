...
class GitHubIntegration:
    ...
    def execute_infrastructure_deployment(self, templates: Dict, intent: Dict) -> Dict:
        ...
        try:
            ...
            if not saved_files:
                return {
                    'response': "❌ No templates were generated",
                    'status': 'error'
                }
            ...
            if commit_result['status'] != 'success':
                return {
                    'response': f"❌ Git commit failed: {commit_result['message']}",
                    'status': 'error'
                }
            ...
            if push_result['status'] == 'success':
                return {
                    'response': f"✅ Infrastructure changes pushed to GitHub!\n🔗 {self.config.get('GITHUB_REPOSITORY')}\n🚀 GitHub Actions will deploy automatically in a few moments.",
                    'status': 'success',
                ...
                return {
                    'response': f"❌ Failed to push to GitHub: {push_result['message']}",
                    'status': 'error'
                }
                
        except Exception as e:
            return {
                'response': f"❌ Deployment error: {str(e)}",
                'status': 'error'
            }
