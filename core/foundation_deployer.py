...
class FoundationDeployer:
    ...
    def deploy_foundation_core(self) -> Dict[str, Any]:
        ...
        # Recursos críticos da Foundation
        core_resources = [
            'dynamodb-state',
            'kms-keys', 
        ...
        for resource in core_resources:
            try:
                print(f"\n🔄 Deploying: {resource}")
                result = deploy_phase_resources('00-foundation', resource)
                results.append({
                    'resource': resource,
                    'result': result
                })
                ...
                if result['successful'] > 0:
                    print(f"   ✅ {result['successful']} resources deployed")
                else:
                    print(f"   ⚠️  No resources deployed")
        ...
        successful_deployments = len([r for r in results if 'error' not in r and r['result']['successful'] > 0])
        
        print(f"\n🎉 Foundation Core Deployment Complete!")
        print(f"   📊 {successful_deployments}/{len(core_resources)} resource groups deployed")
        
        return {
            'core_resources': results,
            'successful_deployments': successful_deployments,
            'total_resource_groups': len(core_resources)
        }
