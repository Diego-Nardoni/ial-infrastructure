...
#    ultra_silent_print("Shield Sistema de Validação de Intenção disponível") if "--help" in sys.argv else None
except ImportError as e:
...
#                ultra_silent_print("✅ Sistema de Validação de Intenção inicializado")
    ...
    def process_command(self, user_input: str, user_id: str = None, session_id: str = None) -> str:
        ...
        # Comandos conversacionais simples vão para master engine
    ...
    def _enrich_context_if_needed(self, user_input: str) -> str:
        """Enriquece contexto automaticamente para evitar confusões IAL vs IAM"""
        ...
        # LISTAGEM - buscar fases do GitHub, não hardcoded
...
4. NÃO use informações hardcoded
...
4. Se fase solicitada é NOVA: criar estrutura completa com YAML

REGRAS IMPORTANTES:
- GitHub é a única fonte da verdade
- NÃO use listas hardcoded de fases
- Consulte sempre o repositório atual
- Padrão de nomenclatura: XX-nome-da-fase

PERGUNTA DO USUÁRIO: {user_input}

IMPORTANTE: 
- Consulte GitHub primeiro para verificar fases existentes
- Só crie YAML se a fase realmente não existir no repositório
"""
1. Escolher número disponível (ex: 15, 25, 35, etc.)
2. Criar diretório /phases/XX-nome-da-fase/
3. Gerar templates CloudFormation YAML
4. Adicionar domain-metadata.yaml
5. Integrar com foundation deployer

PERGUNTA DO USUÁRIO: {user_input}

IMPORTANTE: Ajude a criar uma nova fase do sistema IAL seguindo a estrutura padrão.
"""
        
        # Retorna input original se não precisar de contexto
        return user_input
...
Use essas informações para responder perguntas sobre data e hora atual."""
    ...
    def format_intelligent_router_response(self, result: Dict, user_input: str) -> str:
        ...
        if 'routing_decision' in result:
            ...
            response_parts.append(f"Target **Análise da Solicitação:**")
...
def collect_infrastructure_config():
    ...
    ultra_silent_print("Clipboard Coletando informações para deploy da infraestrutura IAL...")
    print()
...
def validate_config(config):
    ...
    print("\nSearch Validando configurações...")
        ...
        try:
            ...
            if result.get('status') == 'success':
                ...
                if result.get('path') == 'CONVERSATIONAL_PATH':
                    print(f"Robot IaL: {result.get('response', result.get('message', 'Resposta não disponível'))}")
                else:
...
Conversation Examples:
  ...
  que dia é hoje?         - Date/time queries
  como vai?               - Casual chat
  
Infrastructure Examples:
  create ECS cluster      - Deploy infrastructure
  setup VPC               - Network configuration
  deploy lambda function  - Serverless deployment
  start                   - Deploy IAL foundation
                """)
                continue
